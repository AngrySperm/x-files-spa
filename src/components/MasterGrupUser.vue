		...
		<div class="col-12">
			<div class="card">
				...
				<DataTable ref="dt" :value="datas" :lazy="true" v-model:selection="selectedDatas" dataKey="id" :paginator="true" :rows="5" v-model:filters="filters"
					...
					<Column field="name" header="Nama" filterField="name" filterMatchMode="contains" ref="name" :sortable="true" style="width:200px; min-width: 200px;">
				...
				<Dialog v-model:visible="dataDialog" :style="{width: '450px'}" :header="detailDialogTitle" :modal="true" class="p-fluid">
					<div class="field" >
						...
						<InputText id="name" v-model.trim="data.name" required="true" autofocus :class="{'p-invalid': submitted && v$.data.name.$error}" />
						<small class="p-invalid p-error" v-if="submitted && v$.data.name.$error && v$.data.name.required">Nama kosong.</small>
					</div>
					<template #footer>
		...
		searchParent( event ){
			if ( !event.query.trim().length ) {
				...
				this.filteredParents = this.parents.filter((parent) => {
					return parent.name.toLowerCase().includes(event.query.toLowerCase());
				});
			}
		},
	}
