		...
		<div class="col-12">
			<div class="card">
				...
				<DataTable ref="dt" :value="datas" :lazy="true" v-model:selection="selectedDatas" dataKey="id" :paginator="true" :rows="5" v-model:filters="filters"
					...
					<Column field="kode" header="Kode" filterField="kode" filterMatchMode="contains" ref="kode" :sortable="true" style="width:200px; min-width: 200px;">
						<template #filter="{filterModel,filterCallback}">
							<InputText type="text" v-model="filterModel.value" @keydown.enter="filterCallback()" class="p-column-filter" placeholder="Cari kode ..."/>
						</template>
					</Column>
				...
				<Dialog v-model:visible="dataDialog" :style="{width: '450px'}" :header="detailDialogTitle" :modal="true" class="p-fluid">
					...
					<div class="field">
						...
						<InputText id="kode" v-model.trim="data.kode" :class="{'p-invalid': submitted && v$.data.kode.$error}" />
						<small class="p-invalid p-error" v-if="submitted && v$.data.kode.$error && v$.data.kode.required">Kode kosong.</small>
					</div>
					<div class="field">
						<label for="wilayah">Wilayah</label>
						<AutoComplete id="wilayah" v-model="data.wilayah" :suggestions="filteredWilayahs" @complete="searchWilayah($event)" :dropdown="true" field="nama" forceSelection
								:class="{'p-invalid': submitted && v$.data.wilayah.$error}" >
		...
		loadLazyData() {
			...
			try{
				api.getMaster( this.tableFieldName, this.lazyParams ).then( (data) =>{
					...
					// 	this.datas[i].wilayah_id 
					// 	if( this.datas[i].wilayah_id ){
					// 		let o = this.getWilayahById( this.datas[i].wilayah_id );
					// 		this.datas[i]['wilayah'] = o; 
					// 	}else{
					// 		this.datas[i]['wilayah'] = {}; 
					// 	}
					// }
		...
		saveData() {
			...
			try{
				...
				api.saveMaster( this.tableFieldName, this.data ).then( (data) =>{
					if( data.success ){
						if( data.data.wilayah_id ){
							data.data.wilayah = this.getWilayahById( data.data.wilayah_id );
						}
						if( isEdit ){
							for( let i=0; i<this.datas.length; i++ ){
