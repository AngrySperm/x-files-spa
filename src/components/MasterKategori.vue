		...
		<div class="col-12">
			<div class="card">
				...
				<DataTable ref="dt" :value="datas" :lazy="true" v-model:selection="selectedDatas" dataKey="id" :paginator="true" :rows="5" v-model:filters="filters"
					...
					<Column field="kode" header="Kode" filterField="kode" filterMatchMode="contains" ref="kode" :sortable="true">
					...
					<Column field="nama" header="Nama" filterField="nama" filterMatchMode="contains" ref="nama" :sortable="true">
						<template #filter="{filterModel,filterCallback}">
					...
					<!-- <Column selectionMode="multiple" headerStyle="width:0%; min-width: 3rem"></Column>
				...
				<Dialog v-model:visible="dataDialog" :style="{width: '450px'}" :header="detailDialogTitle" :modal="true" class="p-fluid">
					<div class="field">
						...
						<InputText id="nama" v-model.trim="data.nama" required="true" autofocus :class="{'p-invalid': submitted && !data.nama}" />
						<small class="p-invalid" v-if="submitted && !data.nama">Nama kosong.</small>
					</div>
					<div class="field">
						<label for="kode">Kode</label>
						<InputText id="kode" v-model.trim="data.kode" :class="{'p-invalid': submitted && !data.kode}" />
					</div>
					<template #footer>
